  ...
  _createClass(Base, [{
    ...
    value: function render() {
      var _props = this.props,
          ...
          _props$baseStyle = _props.baseStyle,
          baseStyle = _props$baseStyle === undefined ? {} : _props$baseStyle,
          wrap = _props.wrap,
          ...
          props = _objectWithoutProperties(_props, ['style', 'Component', 'baseStyle', 'wrap', 'underlayColor', 'flex', 'row', 'column', 'align', 'height', 'justify']);

      var panza = this.context.panza;

      var _config$panza = _extends({}, _config2.default, panza),
          scale = _config$panza.scale,
          colors = _config$panza.colors,
          borderRadius = _config$panza.borderRadius;

      var sx = [baseStyle, style, margins(props, scale), paddings(props, scale), colorStyle(props, colors, panza), radii(props, borderRadius), flex ? { flex: flex } : null, wrap ? { flexWrap: 'wrap' } : null, column ? { flexDirection: 'column' } : null, row ? { flexDirection: 'row' } : null, align ? { alignItems: align } : null, justify ? { justifyContent: justify } : null, height ? { height: height } : null];
